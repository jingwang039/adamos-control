...
class ExperimentSession:
    ...
    # -------------------------------------------------------------------------
    # Combined: hold indefinitely, allowing the target to change on the fly
    # -------------------------------------------------------------------------

    def hold_continuous(self, initial_target_c,
                        monitor_channel=DEFAULT_MONITOR_CHANNEL,
                        tolerance_c=TOLERANCE_C,
                        settle_time_s=SETTLE_TIME_S,
                        poll_interval_s=POLL_INTERVAL_S,
                        ...
                        stop_event=None):
        """
        Hold the paddle at initial_target_c and keep monitoring/regulating
        until stop_event is set (or the caller hits Ctrl-C). Unlike
        hold_and_verify, this never returns on its own once reached -- it
        keeps running so the setpoint can be changed without restarting.

        get_target : optional zero-arg callable polled once per loop
                     iteration. Whenever it returns a float different from
                     the currently applied setpoint, that new setpoint is
                     applied immediately. Return None for "no change".
        stop_event  : optional threading.Event; set it to exit the loop.
        """
        self._require_paddle()

        current_target = initial_target_c
        ...
        while stop_event is None or not stop_event.is_set():
            if get_target is not None:
                requested = get_target()
                if requested is not None and requested != current_target:
                    try:
                        self.set_target(requested)
                        current_target = requested
                        in_band_since  = None
                        reported       = False
                    except ValueError:
                        pass  # set_target() already logged why it was refused

            paddle_temp = self.get_paddle_temp()
            actual = None
            if use_external:
                try:
                    actual = self.get_channel_temp(monitor_channel)
                    self.logger.info(
                        "paddle internal: %.3f C | %s (external): %.3f C | target: %.3f C",
                        paddle_temp, monitor_channel, actual, current_target)
                except Exception as exc:
                    self.logger.error("Monitor read failed: %s", exc)
            else:
                actual = paddle_temp
                self.logger.info(
                    "paddle internal: %.3f C | target: %.3f C", paddle_temp, current_target)

            errors = self.get_paddle_errors()
            if errors:
                self.logger.error("PTC1 errors: %s", ", ".join(errors))

            if actual is not None:
                if abs(actual - current_target) <= tolerance_c:
                    if in_band_since is None:
                        in_band_since = time.time()
                    elif not reported and time.time() - in_band_since >= settle_time_s:
                        self.logger.info(
                            "Confirmed: surface held within %.1f C of %.3f C for %.0f s.",
                            tolerance_c, current_target, settle_time_s)
                        reported = True
                else:
                    in_band_since = None
                    reported = False

            wait_fn = stop_event.wait if stop_event is not None else time.sleep
            wait_fn(poll_interval_s)
